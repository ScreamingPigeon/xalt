...
static const char *xalt_reasonA[] = {
        ...
        "XALT has found the host does not match hostname pattern. If this is "
        "unexpected check your config.py file: " XALT_CONFIG_PY,
        "XALT has found the executable does not match path pattern. If this is "
        "unexpected check your config.py file: " XALT_CONFIG_PY,
...
static nvmlReturn_t (*_nvmlDeviceGetAccountingPids)(nvmlDevice_t,
                                                    unsigned int *,
                                                    unsigned int *);
static nvmlReturn_t (*_nvmlDeviceGetAccountingStats)(nvmlDevice_t, unsigned int,
...
static dcgmReturn_t (*_dcgmStartEmbedded)(dcgmOperationMode_t opMode,
                                          dcgmHandle_t *pDcgmHandle);
static dcgmReturn_t (*_dcgmJobStartStats)(dcgmHandle_t pDcgmHandle,
                                          dcgmGpuGrp_t groupId, char jobId[64]);
static dcgmReturn_t (*_dcgmJobGetStats)(dcgmHandle_t pDcgmHandle,
                                        char jobId[64],
                                        dcgmJobInfo_t *pJobInfo);
static dcgmReturn_t (*_dcgmJobRemove)(dcgmHandle_t pDcgmHandle, char jobId[64]);
static dcgmReturn_t (*_dcgmJobStopStats)(dcgmHandle_t pDcgmHandle,
                                         char jobId[64]);
static dcgmReturn_t (*_dcgmStopEmbedded)(dcgmHandle_t pDcgmHandle);
static dcgmReturn_t (*_dcgmUpdateAllFields)(dcgmHandle_t pDcgmHandle,
                                            int waitForUpdate);
...
int touch_pid() {
        pid_t xalt_pid = getpid();
        // Can use -tmpdir here
        char *pid_prefix = "/dev/shm/pidXALT";
        char *pid_file_name;

        if (asprintf(&pid_file_name, "%s%d", pid_prefix, xalt_pid) == -1) {
                fprintf(stderr, "Error: Generating pid file failed during "
                                "setup for signal handlers\n");
                return 1;
        }
        ...
        if (pid_file == NULL) {
                fprintf(stderr,
                        "Error: PID file not opening during setup for signal "
                        "handlers %s\n",
                        pid_file_name);
                free(pid_file_name);
                return 1;
        ...
        fclose(pid_file);
        DEBUG(stderr, "    -> Wrote %s for preemption\n", pid_file_name);
        free(pid_file_name);
        return 0;
}
...
void myinit(int argc, char **argv) {
        ...
        /* The SLURM env's must be last.  On Stampede2 both PMI_RANK and
         * SLURM_PROCID are set. Only PMI_RANK is correct with multiple ibrun -n
         * -o */
        /* Lonestar 5, Cray XC-40, only has SLURM_PROCID */
        const char *sizeA[] = {"XALT_COMM_WORLD_SIZE", "OMPI_COMM_WORLD_SIZE",
                               "MV2_COMM_WORLD_SIZE",  "PMI_SIZE",
                               "SLURM_STEP_NUM_TASKS", NULL};
        const char *rankA[] = {"OMPI_COMM_WORLD_RANK", "MV2_COMM_WORLD_RANK",
        ...
        if (xalt_tracing) {
                ...
                fprintf(stderr,
                        ...
                        " Machine:       %s\n"
                        "---------------------------------------------\n\n",
                        dateStr, XALT_GIT_VERSION, u.nodename, u.sysname,
        ...
        DEBUG(stderr,
              "  Test for __XALT_INITIAL_STATE__: \"%s\", STATE: \"%s\"\n",
              (v != NULL) ? v : "(NULL)", STR(STATE));
        /* Stop tracking if another myinit() routine has been called with my pid
         * and hostname*/
        if (v && (strcmp(v, STR(STATE)) != 0)) {
                ...
                if (env_pid && strcmp(env_pid, pid_str) == 0) {
                        DEBUG(stderr,
                              "    -> __XALT_INITIAL_STATE__ has a value: "
                              "\"%s\" -> and it is "
                              "different from STATE: \"%s\" and PID's match: "
                              "%s -> exiting\n}\n\n",
                              v, STR(STATE), env_pid);
        ...
        if (countA[IDX] > 0) {
                DEBUG(stderr,
                      "    -> countA[%d]: %d which is greater than 0 -> "
                      "exiting\n}\n\n",
                      IDX, countA[IDX]);
        ...
        DEBUG(stderr, "  Test for XALT_EXECUTABLE_TRACKING: %s\n",
              (v != NULL) ? v : "(NULL)");
        if (!v || strcmp(v, "yes") != 0) {
                DEBUG(stderr, "    -> XALT_EXECUTABLE_TRACKING is off -> "
                              "exiting\n}\n\n");
        ...
        if (results == SKIP) {
                DEBUG(stderr,
                      "    hostname: \"%s\" is rejected -> exiting\n}\n\n",
                      u.nodename);
        ...
        if (path_results == SKIP) {
                DEBUG(stderr,
                      "    executable: \"%s\" is rejected  -> exiting\n}\n\n",
                      exec_path);
        ...
        if ((build_mask & run_mask) == 0) {
                DEBUG(stderr,
                      "    -> XALT is build to %s, Current %s -> not tracking "
                      "and "
                      "exiting\n}\n\n",
                      xalt_build_descriptA[build_mask],
                      xalt_run_descriptA[run_mask]);
         ...
         *   1) if every character was a utf-16 character that is four bytes
         * converts to 12 (sz*3) 2) Each entry has two quotes and a space
         * (argc*3) 3) There are a pair of square brackets and a null byte (+3)
         */
        ...
        *++p = '\0';
        ...
        if (p > &usr_cmdline[sz]) {
                fprintf(stderr, "XALT: Failure in building user command line "
                                "json string! "
                                "-> exiting\n}\n\n");
  ...
  if (xalt_gpu_tracking)
    {
      ...
      if (result == NVML_SUCCESS)
                  ...
                  if (result == NVML_SUCCESS)
                    {
                      ...
                      if (stats.startTime >= __time)
                        ...
                        /* Mark time the program started.  Use this to compare
                         * to GPU process timestamps in fini to only count
                         * processes that started during the lifetime of the
                         * program. */
                        gettimeofday(&tv, NULL);
                        __time = (unsigned long long)(tv.tv_sec) * 1000000 +
                                 (unsigned long long)(tv.tv_usec);

                        result = _nvmlInit();
                        if (result != NVML_SUCCESS) {
                                DEBUG(stderr,
                                      "    -> Stopping GPU Tracking => Cannot "
                                      "initialize NVML: %s\n\n",
                                      _nvmlErrorString(result));
                                xalt_gpu_tracking = 0;
                                break;
                        ...
                        /* Open the DCGM library at runtime.  This avoids
                        ...
                        DCGMFUNC2(_dcgmStartEmbedded,
                                  DCGM_OPERATION_MODE_MANUAL, &dcgm_handle,
                                  &result);
                        ...
                        result = _dcgmJobStartStats(
                                dcgm_handle, (dcgmGpuGrp_t)DCGM_GROUP_ALL_GPUS,
                                uuid_str);
                        if (result != DCGM_ST_OK) {
                                DEBUG(stderr,
                                      "    -> Stopping GPU Tracking => Cannot "
                                      "start DCGM job stats: "
                                      "%s\n\n",
                                      errorString(result));
                                xalt_gpu_tracking = 0;
                                dcgm_handle = 0;
                        ...
                        result = _dcgmWatchJobFields(
                                dcgm_handle, (dcgmGpuGrp_t)DCGM_GROUP_ALL_GPUS,
                                1000, 1e9, 0);
                        if (result != DCGM_ST_OK) {
                                DEBUG(stderr,
                                      "    -> Stopping GPU Tracking => Cannot "
                                      "start DCGM job watch: "
                                      "%s\n\n",
                                      errorString(result));
                                if (result == DCGM_ST_REQUIRES_ROOT)
                                        DEBUG(stderr,
                                              "    -> May need to enable "
                                              "accounting mode: sudo "
                                              "nvidia-smi -am 1\n");
                                xalt_gpu_tracking = 0;
                                dcgm_handle = 0;
                                break;
        ...
        /* Push XALT_RUN_UUID, XALT_DATE_TIME into the environment so that
         * things like R and python can know what job and what start time of the
         * this run is.
         */
        ...
        // This routine returns either "FALSE" for nothing found or the
        // watermark.
        bool have_watermark = xalt_vendor_note(&watermark, xalt_tracing);
        DEBUG(stderr, "    -> Found watermark via vendor note: %s\n",
              have_watermark ? "true" : "false");

        // If MPI program and no vendor watermark then try extracting the
        // watermark with objdump via extractXALTRecord(...)
        if (num_tasks > 1 && !have_watermark) {
                have_watermark = extractXALTRecordString(exec_path, &watermark);
                DEBUG(stderr, "    -> Found watermark via objdump: %s\n",
                      have_watermark ? "true" : "false");
        ...
        xalt_timer.init = epoch() - t0;
        // Create a start record for any MPI executions with an acceptable
        // number of tasks. or a PKG type
        if (num_tasks >= always_record) {
                DEBUG(stderr,
                      "    -> MPI_SIZE: %d >= MPI_ALWAYS_RECORD: %d => "
                      "recording start "
                      "record!\n",
                      num_tasks, (int)always_record);
                ...
                if (xalt_tracing || xalt_run_tracing) {
                        fprintf(stderr,
                                "  Recording state at beginning of %s user "
                                "program:\n    %s\n",
                                xalt_run_short_descriptA[run_mask], exec_path);
                }

                run_submission(&xalt_timer, orig_pid, ppid, start_time,
                               end_time, probability, exec_path, num_tasks,
                               num_gpus, xalt_run_short_descriptA[xalt_kind],
                               uuid_str, watermark, usr_cmdline, xalt_tracing,
                               always_record, stderr);

                DEBUG(stderr, "    -> uuid: %s\n", uuid_str);
        } else {
                DEBUG(stderr,
                      "    -> MPI_SIZE: %d < MPI_ALWAYS_RECORD: %d, XALT is "
                      "build to %s, "
                      "Current %s -> Not producing a start record\n",
                      num_tasks, (int)always_record,
                      xalt_build_descriptA[build_mask],
                      xalt_run_descriptA[run_mask]);
        }
        ...
        if (strcasecmp(v, "yes") == 0) {
                DEBUG(stderr, "    -> Setting up signals\n");
                int signalA[] = {SIGHUP,  SIGQUIT, SIGILL,  SIGTRAP,
                                 SIGABRT, SIGBUS,  SIGFPE,  SIGTERM,
                                 SIGXCPU, SIGUSR1, SIGUSR2, SIGALRM};
                int signalSz = N_ELEMENTS(signalA);
                struct sigaction action;
                struct sigaction old;
                memset(&action, 0, sizeof(struct sigaction));
                ...
                v = getenv("XALT_RECORD_PIDS");
                if (v != 0) {
                        if (strcasecmp(v, "yes") == 0) {
                                if (touch_pid()) {
                                        DEBUG(stderr, "    -> Writing Pid file "
                                                      "to /dev/shm/"
                                                      "failed \n");
                                }
                        } else {
                                DEBUG(stderr, "XALT TRANSMISSION STYLE IS %s",
                                      v);
                        }
                }
        } else
...
void myfini() {
        ...
        set_end_record(); /* Mark my_free() to not free since we are on the way
                             out */
        ...
        if (orig_pid != getpid()) {
                DEBUG(my_stderr,
                      "    -> exiting because myfini() has been called more "
                      "than once via forking\n}\n\n");
                close_out(my_stderr, xalt_err);
                return;
        }
        ...
        if (signal_hdlr_called)
                DEBUG(my_stderr,
                      "    -> myfini() called via signal handler with signum: "
                      "%d\n",
                      signal_hdlr_called);
        ...
        if (reject_flag != XALT_SUCCESS) {
                ...
                DEBUG(my_stderr,
                      "    -> exiting because reject is set to: %s for "
                      "program: %s\n}\n\n",
                      xalt_reasonA[reject_flag], exec_path);
                close_out(my_stderr, xalt_err);
                return;
        ...
        // Sample all scalar executions and all MPI executions less than
        // **always_record**
        if (num_tasks < always_record) {
                if (xalt_sampling) {
                        double run_time;
                        if (testing_runtime > -0.1) {
                                ...
                                run_time = end_time - start_time;
                        probability =
                                prgm_sample_probability(num_tasks, run_time);

                        if (my_rand >= probability) {
                                DEBUG(my_stderr,
                                      "    -> exiting because sampling. "
                                      "run_time: %g, (my_rand: %g > prob: %g) "
                                      "for program: %s\n}\n\n",
                                      run_time, my_rand, probability,
                                      exec_path);
                                if (xalt_err && my_stderr) {
                                        fclose(my_stderr);
                                        close(errfd);
                                ...
                                DEBUG(my_stderr,
                                      "    -> Sampling program run_time: %g: "
                                      "(my_rand: %g <= prob: %g) "
                                      "for program: %s\n",
                                      run_time, my_rand, probability,
                                      exec_path);
                } else
                        DEBUG(my_stderr, "    -> XALT_SAMPLING = \"no\" All "
                                         "programs tracked!\n");
        }
        ...
        if (xalt_gpu_tracking) {
                ...
                if (result == NVML_SUCCESS) {
                        ...
                        for (i = 0; i < device_count; i++) {
                                ...
                                /* Get a device handle */
                                result =
                                        _nvmlDeviceGetHandleByIndex(i, &device);
                                if (result != NVML_SUCCESS) {
                                        DEBUG(my_stderr,
                                              "  Unable to get device handle "
                                              "for GPU %d: %s\n",
                                              i, _nvmlErrorString(result));
                                        continue;
                                }

                                /* Check whether accounting mode is enabled for
                                 * this device */
                                result = _nvmlDeviceGetAccountingMode(device,
                                                                      &mode);
                                if (result != NVML_SUCCESS) {
                                        DEBUG(my_stderr,
                                              "  Unable to get accounting mode "
                                              "for GPU %d: %s\n",
                                              i, _nvmlErrorString(result));
                                        continue;
                                }
                                if (mode != NVML_FEATURE_ENABLED) {
                                        DEBUG(my_stderr,
                                              "  Accounting mode is not "
                                              "enabled for GPU %d. Enable "
                                              "accounting mode: sudo "
                                              "nvidia-smi -i %d -am 1\n",
                                              i, i);
                                        continue;
                                ...
                                /* Get the size of the accounting buffer */
                                result = _nvmlDeviceGetAccountingBufferSize(
                                        device, &max_pid_count);
                                if (result != NVML_SUCCESS) {
                                        DEBUG(my_stderr,
                                              "  Unable to get the accounting "
                                              "buffer size for GPU %d: %s\n",
                                              i, _nvmlErrorString(result));
                                        continue;
                                }

                                /* Allocate space for the accounting data */
                                pids = (unsigned int *)XMALLOC(
                                        sizeof(unsigned int) * max_pid_count);
                                memset(pids, 0,
                                       sizeof(unsigned int) * max_pid_count);

                                /* Get PID accounting data */
                                pid_count = max_pid_count;
                                result = _nvmlDeviceGetAccountingPids(
                                        device, &pid_count, pids);
                                if (result != NVML_SUCCESS) {
                                        DEBUG(my_stderr,
                                              "  Unable to get accounting data "
                                              "for GPU %d: %s\n",
                                              i, _nvmlErrorString(result));
                                        my_free(pids, sizeof(unsigned int) *
                                                              max_pid_count);
                                        continue;
                                }

                                /* Loop over the PID accounting data looking for
                                 * PIDs that started after the program. Count
                                 * those as belonging to this program execution.
                                 */
                                unsigned int j = 0;
                                for (j = 0; j < pid_count; j++) {
                                        nvmlAccountingStats_t stats;

                                        result = _nvmlDeviceGetAccountingStats(
                                                device, pids[j], &stats);
                                        if (result == NVML_SUCCESS) {
                                                /* If the GPU PID start time is
                                                   later than the program start
                                                   time, consider the GPU PID to
                                                   belong to the program.  This
                                                   would not necessarily be true
                                                   if the system is shared by
                                                   unrelated programs. */
                                                if (stats.startTime >= __time) {
                                                        /* There is a mystery
                                                           PID that appears for
                                                           all GPUs.  The PID is
                                                           constant and is
                                                           assumed to be a CUDA
                                                           cleanup process. That
                                                           PID has a
                                                           maxMemoryUsage of 0
                                                           while a real PID will
                                                           always have a
                                                           non-zero value, so
                                                           use that to exclude
                                                           it. */
                                                        if (stats.maxMemoryUsage >
                                                            0) {
                                                                num_active_pids++;
                                                                DEBUG(my_stderr,
                                                                      "  PID "
                                                ...
                                                /* Note that the GPU compute
                                                   process has not terminated
                                                   yet.  Therefore
                                                   stats.isRunning == 1, and
                                                   stats.time == 0, so it is not
                                                   possible to get the amount of
                                                   time the GPU device was
                                                   actually in use. */
                                        }
                                }
                                my_free(pids,
                ...
                if (dcgm_handle != 0) {
                        ...
                        result = _dcgmJobGetStats(dcgm_handle, uuid_str,
                                                  &job_info);
                        if (result == DCGM_ST_OK) {
                                int i = 0;
                                DEBUG(my_stderr, "  %d GPUs detected\n",
                                ...
                                for (i = 0; i < job_info.numGpus; i++) {
                                        DEBUG(my_stderr,
                                              "  GPU %d: num compute pids %d\n",
                                              i,
                                              job_info.gpus[i].numComputePids);
        ...
        if (xalt_tracing || xalt_run_tracing)
                DEBUG(my_stderr,
                      "  Recording State at end of %s user program\n",
                      xalt_run_short_descriptA[run_mask]);

        if (my_stderr)
                fflush(my_stderr);
        xalt_timer.fini = epoch() - t0;
        run_submission(&xalt_timer, orig_pid, ppid, start_time, end_time,
                       probability, exec_path, num_tasks, num_gpus,
                       xalt_run_short_descriptA[xalt_kind], uuid_str, watermark,
...
static int load_nvml() {
        ...
        if (!nvml_dl_handle) {
                ...
                if (!nvml_dl_handle) {
                        ...
                        if (!nvml_dl_handle) {
                                DEBUG(stderr,
                                      "    -> Unable to open libnvidia-ml.so "
        ...
        *(void **)(&_nvmlDeviceGetCount) =
                dlsym(nvml_dl_handle, "nvmlDeviceGetCount");
        *(void **)(&_nvmlDeviceGetHandleByIndex) =
                dlsym(nvml_dl_handle, "nvmlDeviceGetHandleByIndex");
        *(void **)(&_nvmlErrorString) =
                dlsym(nvml_dl_handle, "nvmlErrorString");
        ...
        *(void **)(&_nvmlShutdown) = dlsym(nvml_dl_handle, "nvmlShutdown");
        if (xalt_tracing) {
                ...
                DEBUG(stderr,
                      "    -> Successfully dynamically linked with nvidia-ml "
                      "library: %s\n",
                      name);
        }
...
static int load_dcgm() {
        ...
        *(void **)(&_dcgmStartEmbedded) =
                dlsym(dcgm_dl_handle, "dcgmStartEmbedded");
        *(void **)(&_dcgmStopEmbedded) =
                dlsym(dcgm_dl_handle, "dcgmStopEmbedded");
        *(void **)(&_dcgmJobStartStats) =
                dlsym(dcgm_dl_handle, "dcgmJobStartStats");
        *(void **)(&_dcgmJobGetStats) =
                dlsym(dcgm_dl_handle, "dcgmJobGetStats");
        *(void **)(&_dcgmJobRemove) = dlsym(dcgm_dl_handle, "dcgmJobRemove");
        *(void **)(&_dcgmJobStopStats) =
                dlsym(dcgm_dl_handle, "dcgmJobStopStats");
        *(void **)(&_dcgmUpdateAllFields) =
                dlsym(dcgm_dl_handle, "dcgmUpdateAllFields");
        *(void **)(&_dcgmWatchJobFields) =
                dlsym(dcgm_dl_handle, "dcgmWatchJobFields");
        if (xalt_tracing) {
                struct link_map *map;
                int result = dlinfo(dcgm_dl_handle, RTLD_DI_LINKMAP, &map);
                char *name = realpath(map->l_name, NULL);
                DEBUG(stderr,
                      "    -> Successfully dynamically linked with dcgm "
                      "library: %s\n",
                      name);
        }
        return 1;
...
__attribute__((section("__DATA,__mod_init_func"), used,
               aligned(sizeof(void *)))) __typeof__(myinit) *__init = myinit;
__attribute__((section("__DATA,__mod_term_func"), used,
               aligned(sizeof(void *)))) __typeof__(myfini) *__fini = myfini;
#else
